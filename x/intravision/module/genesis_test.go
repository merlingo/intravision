package intravision_test

import (
	"testing"

	keepertest "github.com/merlingo/intravision/testutil/keeper"
	"github.com/merlingo/intravision/testutil/nullify"
	intravision "github.com/merlingo/intravision/x/intravision/module"
	"github.com/merlingo/intravision/x/intravision/types"
	"github.com/stretchr/testify/require"
)

func TestGenesis(t *testing.T) {
	genesisState := types.GenesisState{
		Params: types.DefaultParams(),

		// this line is used by starport scaffolding # genesis/test/state
	}

	k, ctx := keepertest.IntravisionKeeper(t)
	intravision.InitGenesis(ctx, k, genesisState)
	got := intravision.ExportGenesis(ctx, k)
	require.NotNil(t, got)

	nullify.Fill(&genesisState)
	nullify.Fill(got)

	// this line is used by starport scaffolding # genesis/test/assert
}
