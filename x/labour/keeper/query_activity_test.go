...
func TestActivityQueryPaginated(t *testing.T) {
	...
	t.Run("Total", func(t *testing.T) {
		...
		act_msgs := []types.Activity{}
		for i := range msgs {
			if msgs[i].TaskId == testutil.TaskID && msgs[i].Worker == testutil.Alice {
				act_msgs = append(act_msgs, msgs[i])
			}
		}
		require.NoError(t, err)
		require.Equal(t, len(msgs), int(resp.Pagination.Total))
		require.Equal(t, 2, len(resp.Activity))
		require.ElementsMatch(t,
			nullify.Fill(act_msgs),
			nullify.Fill(resp.Activity),
		)
	})
